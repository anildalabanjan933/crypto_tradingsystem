...
if not pos.get("success"):
    ...
    from engine.telegram_alert import send_alert
    send_alert("CTS S4V2 CRITICAL: Startup position sync failed after 5 retries. Bot BLOCKED - manual check required before restart.")
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            if _bt_ep2 > 0 and _entry_price_for_alert > 0 and _exit_fill_price > 0:
                                ...
                                _append_fill_log("logs/fill_prices_s4v2.csv", sig_ts, _xt, dirn, lots, _bt_ep2, _entry_price_for_alert, _bt_xp2, _exit_fill_price, _total_charges)
                        else:
            ...
            # --- ENTRY if no position and exit time not yet reached ---
                ...
                if _entry_retry_state["count"] >= _ENTRY_MAX_ATTEMPTS:
                    ...
                    if not check_engine_heartbeat():
                        ...
                        _cid = 'S4V2E' + sig_ts.replace('-','').replace(':','')
