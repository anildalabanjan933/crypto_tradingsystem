                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            if _entry_price_for_alert > 0 and _exit_fill_price > 0 and _bt_ep2 > 0:
                                _send_roundtrip_match_alert("S4V2", dirn, _entry_price_for_alert, _exit_fill_price, _bt_ep2, _bt_xp2 if _bt_xp2 > 0 else _bt_ep2, lots, entry_ts=sig_ts, exit_ts=_xt, total_charges=float(_entry_commission_for_log)+float(result.get("commission",0.0)))
