    ...
    try:
        ...
        if _matched:
                ...
                if _ex_size == 0:
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            bt_row = _get_bt_trade(_xt, "S2")
                            _msg = _build_alert("EXIT","S2",dirn,sig_ts,0,_xt,0,0,bt_row)
                            send_alert(_msg)
                        else:
            ...
            # --- ENTRY if no position and exit time not yet reached ---
                ...
                _send_live_entry_alert('S2', _dir, _et, float(_entry_fill), float(_sl_price) if '_sl_price' in dir() else 0.0)
                _send_match_alert('S2', _dir, float(_bt_ep) if '_bt_ep' in dir() else 0.0, float(_entry_fill), _et)
