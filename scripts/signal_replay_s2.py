...
def _get_match_count():
    try:
        v = open("logs/match_count.txt").read().strip()
        return int(v)
    except:
        return 0

def _increment_match_count():
    c = _get_match_count() + 1
    open("logs/match_count.txt","w").write(str(c))
    return c
...
while True:
    try:
        ...
        # --- Check live_signal_s2.txt (engine single source of truth) ---
        ...
        if _live_sig and _live_sig.get("seq", 0) > last_processed_seq:
            ...
            if not _live_matched:
                log.info(f"[LIVE] Engine signal {_live_ts} not in CSV yet - waiting for CSV update")
                ...
                if _ex_size == 0:
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            # Alert 4: Live exit filled
                            _send_live_exit_alert("S2", dirn, _xt, _exit_fill_price, open_entry_price, lots)
                            ...
                            if _bt_ep2 > 0 and open_entry_price > 0 and _exit_fill_price > 0:
                                _send_roundtrip_match_alert("S2", dirn, open_entry_price, _exit_fill_price, _bt_ep2, _bt_xp2, lots)
                        else:
                            log.error(f"[ORDER] EXIT FAILED: {result}")
                            send_alert(f"CTS S2 EXIT FAILED\nError: {result}")
                            last_known_ts = load_ts_file(TS_FILE)
