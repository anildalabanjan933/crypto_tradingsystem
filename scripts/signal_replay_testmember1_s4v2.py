...
TS_FILE      = "logs/last_known_ts_testmember1_s4v2.txt"
BASELINE_FILE= "logs/valid_from_baseline.txt"
SLEEP_SEC    = 1
LOG_FILE     = "logs/live_trading_testmember1_s4v2.log"
...
# --- Order Manager ---
API_KEY    = os.getenv("TESTMEMBER1_S4V2_API_KEY", "")
API_SECRET = os.getenv("TESTMEMBER1_S4V2_API_SECRET", "")
...
try:
    ...
    if _missed:
        for _ms in _missed:
            ...
            send_alert(
                f"⚠️ CTS TM1 S4V2 BOT RESTART - MISSED TRADE\n"
                f"━━━━━━━━━━━━━━━━━━\n"
...
def check_engine_heartbeat():
    ...
    try:
        ...
        if age_min > 15:
            ...
            from engine.telegram_alert import send_alert
            send_alert(f"CTS TM1 S4V2 WARNING - Engine heartbeat stale {int(age_min)}m - orders blocked until engine restarts")
            return False
        return True
    except Exception as e:
...
while True:
    try:
        ...
        # --- Check logs/live_signal_s4v2.txt first (1-2 sec from engine) ---
        _live_sig = read_live_signal("logs/live_signal_s4v2.txt")
        _live_matched = None
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            if _bt_ep2 > 0 and _entry_price_for_alert > 0 and _exit_fill_price > 0:
                                _send_roundtrip_match_alert("S4V2", dirn, _entry_price_for_alert, _exit_fill_price, _bt_ep2, _bt_xp2, lots)
                        else:
                            log.error(f"[ORDER] EXIT FAILED: {result}")
                            send_alert(f"CTS TM1 S4V2 EXIT FAILED\nError: {result}")
                            last_known_ts = load_ts_file(TS_FILE)
            ...
            # --- ENTRY if no position and exit time not yet reached ---
                ...
                if not check_engine_heartbeat():
                    ...
                    if result.get("success"):
                        ...
                        _send_live_entry_alert("S4V2", direction, sig_ts, real_entry, _sl_price_val, lots)
                        ...
                        send_alert(f"CTS TM1 S4V2 ENTRY FAILED\nError: {result}")
                        last_known_ts = load_ts_file(TS_FILE)
