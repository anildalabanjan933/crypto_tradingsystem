                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            if _bt_ep2 > 0 and _entry_price_for_alert > 0 and _exit_fill_price > 0:
                                _send_roundtrip_match_alert("S4V3", dirn, _entry_price_for_alert, _exit_fill_price, _bt_ep2, _bt_xp2, lots)
                                _exit_commission = result.get("commission", 0.0)
                                _total_charges = float(_entry_commission_for_log) + float(_exit_commission)
                                _append_fill_log("logs/fill_prices_s4v3.csv", sig_ts, _xt, dirn, lots, _bt_ep2, _entry_price_for_alert, _bt_xp2, _exit_fill_price, _total_charges)
                        else:
