...
try:
    ...
    if _missed:
        for _ms in _missed:
            ...
            send_alert(
                f"⚠️ CTS TM1_S2 BOT RESTART - MISSED TRADE\n"
                f"━━━━━━━━━━━━━━━━━━\n"
...
while True:
    try:
        ...
        # --- Check logs/live_signal_s2.txt first (1-2 sec from engine) ---
        _live_sig = read_live_signal("logs/live_signal_s2.txt")
        _live_matched = None
            ...
            if position is None and now >= _xt:
                ...
                if _ex_size == 0:
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            _send_live_exit_alert("TM1_S2", dirn, _xt, _exit_fill_price, open_entry_price, lots)
                            _bt_csv2 = _get_csv_bt_row("TM1_S2", sig_ts)
                            ...
                            if _bt_ep2 > 0 and open_entry_price > 0 and _exit_fill_price > 0:
                                _send_roundtrip_match_alert("TM1_S2", dirn, open_entry_price, _exit_fill_price, _bt_ep2, _bt_xp2, lots)
                        else:
                            log.error(f"[ORDER] EXIT FAILED: {result}")
                            send_alert(f"CTS TM1_S2 EXIT FAILED\nError: {result}")
                            last_known_ts = load_ts_file(TS_FILE)
                ...
                if not check_engine_heartbeat():
                    ...
                    if result.get("success"):
                        ...
                        _send_live_entry_alert("TM1_S2", direction, sig_ts, real_entry, _sl_price_val, lots)
                        _bt_csv = _get_csv_bt_row("TM1_S2", sig_ts)
                        ...
                        if _bt_ep > 0 and real_entry > 0:
                            _send_entry_match_alert("TM1_S2", direction, sig_ts, _bt_ep, real_entry, _bt_xt, _bt_xp, lots)
                        open_entry_price = real_entry
                        log.info(f"[ORDER] ENTRY confirmed | position={position}")
                    else:
                        log.error(f"[ORDER] ENTRY FAILED: {result}")
                        send_alert(f"CTS TM1_S2 ENTRY FAILED\nError: {result}")
                        last_known_ts = load_ts_file(TS_FILE)
