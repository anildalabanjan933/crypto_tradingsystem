...
# --- Order Manager ---
API_KEY    = os.getenv("TESTMEMBER1_S4_API_KEY", "")
API_SECRET = os.getenv("TESTMEMBER1_S4_API_SECRET", "")
...
if position is not None:
    try:
        open_entry_price = float(open("logs/entry_price_testmember1_s4.txt").read().strip())
...
try:
    ...
    if _missed:
        for _ms in _missed:
            ...
            send_alert(
                f"⚠️ CTS TM1_S4 BOT RESTART - MISSED TRADE\n"
                f"━━━━━━━━━━━━━━━━━━\n"
...
def check_engine_heartbeat():
    ...
    try:
        ...
        if age_min > 15:
            ...
            from engine.telegram_alert import send_alert
            send_alert(f"CTS MEMBER WARNING - Engine heartbeat stale {int(age_min)}m - orders blocked until engine restarts")
            return False
        return True
    except Exception as e:
...
while True:
    try:
        open('logs/heartbeat_testmember1_s4.txt','w').write(str(__import__('time').time()))
    except Exception:
        pass
    try:
        now = now_utc_str()
                ...
                if _self_heal_due:
                    ...
                    if _ex_size == 0:
                        ...
                        if result.get("success"):
                            ...
                            save_ts_file(TS_FILE, _next_row["entry_time"])
                            last_known_ts = safe_ts(_next_row["entry_time"])
                            send_alert(f"CTS TM1_S4 SELF-HEAL: Orphaned PENDING exit auto-closed | entry={sig_ts} | advanced to next_signal={_next_row['entry_time']}")
                    ...
                    if not check_engine_heartbeat():
                        ...
                        if result.get("success"):
                            ...
                            if _bt_ep2 > 0 and _entry_price_for_alert > 0 and _exit_fill_price > 0:
                                _send_roundtrip_match_alert("TM1_S4", dirn, _entry_price_for_alert, _exit_fill_price, _bt_ep2, _bt_xp2, lots, entry_ts=sig_ts, exit_ts=_xt, total_charges=float(_entry_commission_for_log)+float(result.get("commission",0.0)), bt_entry_ts=_bt_csv2[0] if _bt_csv2 and len(_bt_csv2)>0 else None, bt_exit_ts=_bt_csv2[1] if _bt_csv2 and len(_bt_csv2)>1 else None, bt_direction=_bt_csv2[2] if _bt_csv2 and len(_bt_csv2)>2 else None)
            ...
            # --- ENTRY if no position and exit time not yet reached ---
                ...
                if _entry_retry_state["count"] >= _ENTRY_MAX_ATTEMPTS:
                    ...
                    if not check_engine_heartbeat():
                        ...
                        _cid = 'TM1E' + sig_ts.replace('-','').replace(':','') + f'_a{_entry_retry_state["count"]}'
                        ...
                        if result.get("success"):
                            ...
                            try:
                                with open("logs/entry_price_testmember1_s4.txt","w") as _epf:
