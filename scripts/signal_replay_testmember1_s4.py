        ...
        for sig in signals:
            ...
            if now >= entry_time and position is None:
                ...
                if result.get("success"):
                    ...
                    send_alert(f"CTS MEMBER_S4 ENTRY\nDirection: {direction.upper()}\nLots: {lots}")
                else:
                    log.error(f"[ORDER] ENTRY FAILED: {result}")
                    send_alert(f"CTS MEMBER_S4 ENTRY FAILED\nError: {result}")
                    if result.get('error',{}).get('code') == 'invalid_api_key':
                        log.error("[CRITICAL] invalid_api_key - check API key and IP whitelist")
                        send_alert("CTS MEMBER_S4 CRITICAL: invalid_api_key - check API key and IP whitelist")
                    last_known_ts = load_ts_file(TS_FILE)
                break
            ...
            if now >= exit_time:
                ...
                if result.get("success"):
                    ...
                    send_alert(f"CTS MEMBER_S4 EXIT\nPosition closed\nLots: {close_size}")
                else:
