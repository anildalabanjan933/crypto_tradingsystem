    ...
    try:
        ...
        if _matched:
            ...
            if position is None and now >= _xt:
                ...
                _send_live_entry_alert('TM1_S4', _dir, _et, float(_entry_fill), float(_sl_price) if '_sl_price' in dir() else 0.0)
                _send_match_alert('TM1_S4', _dir, float(_bt_ep) if '_bt_ep' in dir() else 0.0, float(_entry_fill), _et)
