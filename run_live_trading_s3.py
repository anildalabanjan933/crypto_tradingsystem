# run_live_trading_s3.py
# Responsibility: 24/7 live trading loop for BTC Renko + Breakout strategy (S3)
# Runs on Delta Exchange testnet (S3RenkoPullBr sub-account)
#
# Strategy rules:
#   Entry : 1 box close above/below trendline/support/resistance (min 2 touches)
...
API_KEY        = "vM13QTAkEjRnLp9nnkWuZPcmqU2acl"
API_SECRET     = "JF3D79bL9RxKxyKHLtQj0My8KdY7NHb6Eea7aFYkOWb7oFPGrypmzef63nKU"
...
SWING_L          = 2
SWING_R          = 2
SR_TOLERANCE     = 1.0         # S3 breakout uses 1.0
MIN_AGE_HORIZ    = 30
COOLDOWN_BOXES   = 20
ANCHOR_LOOKBACK  = 50
...
# Logging
LOG_FILE = "logs/live_trading_s3.log"
...
log = logging.getLogger("LiveTrading_S3")
...
def run_live():
    log.info("=" * 60)
    log.info("S3 RenkoBreakout Live Trading Bot STARTING")
    log.info(f"Mode      : {'TESTNET' if TESTNET else 'LIVE'}")
    log.info(f"Account   : S3RenkoPullBr")
    ...
    log.info("[Loop] S3 RenkoBreakout bot stopped.")
