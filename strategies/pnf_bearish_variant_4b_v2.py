"""
PnF Bearish Variant 4B — V2 TEST VERSION
==========================================
Change from Test version (pnf_bearish_variant_4b_test.py):
  ADDED: Trendline check as Condition 0 (before Double Bottom)
  RULE:  Minimum 3 strictly rising O bottoms required for valid trendline
         Entry blocked if fewer than 3 anchors exist
         Entry blocked if current O end_level >= projected trendline value

Difference from original (pnf_bearish_variant_4b.py):
  REMOVED: Supertrend entry filter (upper_band > price check)
  KEPT:    Supertrend upper_band as SL source after entry
...
class PnFBearishVariant4BV2(BaseStrategy):
    """
    V2 TEST — Trendline check restored (min 3 rising O bottoms).
    Supertrend entry filter still removed (same as Test version).
    """
    ...
    def __init__(self, data_dict: Dict, lot_size: int = 1, **kwargs):
        ...
        self.trendline_min_anchors = kwargs.get('trendline_min_anchors',   3)
    ...
    def get_name(self) -> str:
        return "PnF Bearish Variant 4B [V2 - Trendline Min 3 Anchors]"

    def get_description(self) -> str:
        return (
            "V2 TEST: Trendline restored with minimum 3 rising O bottoms. "
    ...
    def generate_signals(self) -> List[Dict]:
        ...
        for col_idx in range(len(columns)):
            ...
            # ── Condition 0: Trendline break (min 3 rising O anchors) ──
        ...
        # ── Debug summary ──────────────────────────────────────────────
        print(f"\n  === SIGNAL FILTER DEBUG [V2 - MIN 3 ANCHORS] ===")
