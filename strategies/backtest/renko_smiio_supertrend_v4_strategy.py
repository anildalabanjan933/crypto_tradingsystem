...
# STRATEGY CLASS
# ===========================================================================
class RenkoSMIIOSupertrendV4Strategy(BaseStrategy):
    """
    ...
    def required_timeframes(self) -> list:
        return [self.renko_timeframe, '1m']
    ...
    def _build_renko_df(self) -> pd.DataFrame:
        ...
        # --- V4 CHANGE: source close price 1 minute before boundary (max precision available in 1m BT data) ---
        # Boundary timestamp label stays the same as original BT (matches display format).
        df_1m = self._data.get('1m')
        if df_1m is not None and len(df_1m) > 0:
            ts_1m = (df_1m.index if isinstance(df_1m.index, pd.DatetimeIndex)
                     else pd.to_datetime(df_1m['timestamp']))
            close_1m_series = pd.Series(df_1m['close'].values, index=pd.DatetimeIndex(ts_1m)).sort_index()
            early_closes = closes.copy()
            for _i, _boundary_ts in enumerate(timestamps):
                _early_ts = pd.Timestamp(_boundary_ts) - pd.Timedelta(minutes=1)
                _avail = close_1m_series[close_1m_series.index <= _early_ts]
                if len(_avail) > 0:
                    early_closes[_i] = _avail.iloc[-1]
            closes = early_closes
