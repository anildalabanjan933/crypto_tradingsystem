...
with _tab_audit:
    from dashboard.trade_audit_tab import (
        ...
        _read_log_lines_audit,
        _fetch_account_orders_history_cached_audit,
    )
...
with _tab_analysis:
    ...
    with st.expander("SECTION 13 - LIVE FORWARD TEST PERFORMANCE", expanded=st.session_state.get('exp_13s', False)):
        if not st.session_state.get('exp_13s', False):
            st.caption("Collapsed - expand to load live forward-test performance data")
        else:
                ...
                try:
                    ...
                    for p in pairs:
                        ...
                        rows13.append({"direction":p.get('dir',''), "lot":p.get('sz',0), "entry_time_utc":_et13,
                                       ...
                                       "net_pnl_usd":p.get('pnl',0), "net_pnl_inr":round(p.get('pnl',0)*_INR13,2), "charges_usd":p.get('cm',0), "status":"OPEN" if p.get('open') else "CLOSED", "message":p.get('_msg13_csv','')})
        ...
        def _today_trades_html(df2, df4, df2_fwd, df4_fwd, df3=None, df3_fwd=None):
                    ...
                    if _os_ti.path.exists(_sl_path):
                        ...
                        for _line in _read_log_lines_cached(_sl_path, _sl_mtime):
                            ...
                            if "AUTO-PLACED SUCCESS" in _line or "RECOVERED - SL WAS MISSING" in _line:
                                _issues.append("[CDS SYSTEM SIDE] SL was missing, auto-fixed within 60s. Trade stayed safe.")
                            elif "ORPHAN POSITION" in _line:
                                _issues.append("[CDS SYSTEM SIDE] Position was untracked, already auto-fixed same day.")
                            elif "STUCK PENDING" in _line:
                                _issues.append("[CDS SYSTEM SIDE] Sync was briefly delayed, fixed itself. No action needed.")
                            elif "EMERGENCY CLOSE" in _line:
                                _issues.append("[CDS SYSTEM SIDE] SL failed to place, trade auto-closed for safety.")
                    ...
                    if _os_ti.path.exists(_eng_path):
                        ...
                        for _line in _read_log_lines_cached(_eng_path, _eng_mtime):
                            ...
                            if _window_start <= _line_ts <= _window_end:
                                _issues.append("Short connection drop, reconnected in 5s. No real issue.")
                    ...
                    if _os_ti.path.exists(_bot_log_path):
                        ...
                        for _line in _read_log_lines_cached(_bot_log_path, _bot_log_mtime):
                            ...
                            try:
                                ...
                                continue
                            if _window_start <= _line_ts <= _window_end:
                                _issues.append(f"Bot restarted around {_line_ts.strftime('%H:%M:%S')} UTC. May have caused delay/miss.")
                    if _os_ti.path.exists(_bot_log_path):
                        for _line in _read_log_lines_cached(_bot_log_path, _bot_log_mtime):
                            _ts_raw = _line[:23]
                            try:
            ...
            def _match(bt, lv):
                ...
                if _issue_msgs:
                    ...
                    _issue_html = "<br>" + "<br>".join([f"<span style='font-size:9px;color:#000;font-weight:400;'>⚠ {_tag_or_default(m)}</span>" for m in _issue_msgs])
            ...
            def _section_html(strat, bt_rows, lv_rows):
                ...
                tbl += f'<th style="{TH}width:6%;">Entry $</th>'
                tbl += f'<th style="{TH}width:6%;">Exit $</th>'
                tbl += f'<th style="{TH}width:5%;">PnL $</th>'
                tbl += f'<th style="{TH}width:11%;">Net PnL ₹ ($5/side)</th>'
                tbl += f'<th style="{TH}width:11%;">Net PnL ₹ ($10/side)</th>'
                tbl += f'<th style="{TH}width:4%;">Charges ₹</th>'
                tbl += f'<th style="{TH}width:10%;">Match</th>'
                tbl += f'<th style="{TH}width:28%;">Message</th>'
                tbl += '</tr></thead><tbody>'
                if tc == 0:
...
def _month_trades_html(df2, df4, df2_fwd, df4_fwd):
        ...
        try:
            ...
            if _osm.path.exists(_enp):
                for _line in _rll_m(_enp, _osm.path.getmtime(_enp)):
                    ...
                    if _ws <= _lt <= _we: _issues.append("Short connection drop, reconnected in 5s. No real issue.")
    ...
    def _match_m(bt, lv, tf_min):
        ...
        _issue_html = ""
        if _issue_msgs:
            _issue_html = "<br>" + "<br>".join([f"<span style='font-size:11px;color:#000;font-weight:400;'>⚠ {m}</span>" for m in _issue_msgs])
                ...
                if r is None:
                    ...
                    if is_lv:
                        try:
                            ...
                            _miss_issues3 = []
                        if _miss_issues3:
                            _miss += "<br>" + "<br>".join([f"<span style='font-size:10px;color:#000;font-weight:400;'>⚠ {m}</span>" for m in _miss_issues3])
                        else:
