...
for log in logs/live_trading_testmember1_s4v2.log logs/live_trading_testmember1_s4.log logs/live_trading_s4v2.log logs/live_trading_s4.log; do
...
screen -S live_s4v2 -X quit 2>/dev/null
...
screen -dmS live_s4v2 bash -c "cd $REPO && set -a && source $REPO/.env && set +a && $REPO/.venv/bin/python3 scripts/signal_replay_s4v2.py"
...
screen -dmS sl_safety_monitor bash -c "cd $REPO && set -a && source $REPO/.env && set +a && $REPO/.venv/bin/python3 scripts/sl_safety_monitor.py"
screen -dmS position_risk_monitor bash -c "cd $REPO && set -a && source $REPO/.env && set +a && $REPO/.venv/bin/python3 scripts/position_risk_monitor.py"
